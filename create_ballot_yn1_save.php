<?php
   ...
   */
   ...
   if(isset($_POST['tfyn']))   {
      if(!empty($_POST['tfyn']))  {
         $tfyn = $_POST['tfyn'];
         $_SESSION['ballottype'] = $tfyn;
      }  else  {
         $tfyn = "3";   //3 means yes/no, 4 means true/false.
         $_SESSION['ballottype'] = $tfyn;
      }
   }  else  {
         $tfyn = "3";   //3 means yes/no, 4 means true/false.
   ...
   */
   ...
   function insert_ballot_main($userid, $electionname, $electionfinaldate, $district, $description, $ballottype, $allvoters, $infolink, $keyword)  {
      ...
      $query = "insert into electionlist_main
               ...
               values(?, ?, ?, ?, ?, ?, ?, ?, ?)";
      echo "ins query: $query<br/>";
      echo "uid: $userid, ename: $electionname, edate: $electionfinaldate, district: $district, desc: $description, baltype: $ballottype, allvoters: $allvoters, infolink: $infolink, keyword: $keyword";
